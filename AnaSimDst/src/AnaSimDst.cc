	  ...
	  if(mi_sqhitvec) {
		  for(int ihit=0; ihit<mi_sqhitvec->size(); ++ihit) {
			  ...
			  if(hit->get_detector_id() >= det_ID1 and hit->get_detector_id() <=det_ID2) {
				  ...
				  cout <<"charge: "<<trk->get_charge()<<"     get_track_id: "<<hit->get_track_id() <<"     get_detector_id () : "<< hit->get_detector_id () <<  "     getDetectorName: " << geom->getDetectorName( hit->get_detector_id () ) <<endl;
