...
int AnaEffHodo::Init(PHCompositeNode* topNode)
{
  ...
  std::string paddleNames[16] = {"H2T1", "H2T2", "H2T3", "H2T4", "H2T5", "H2T6", "H2T7", "H2T8","H2T9","H2T10","H2T11","H2T12", "H2T13", "H2T14", "H2T15", "H2T16"};
  char buffer[20];
 
  for(int i = 0; i < 16; ++i)  {       
	  sprintf(buffer, "%s_all", paddleNames[i].c_str());
	  h2xt_all[i] = new TH1D(buffer, buffer, 10, 0.5, 10.5);

	  sprintf(buffer, "%s_acc", paddleNames[i].c_str());
	  h2xt_acc[i] = new TH1D(buffer, buffer, 10, 0.5, 10.5);

	  sprintf(buffer, "%s_eff", paddleNames[i].c_str());
          HodopaddleNames[i] = new TH1D(buffer, buffer, nElements[i], 0.5, nElements[i]+0.5);
  }
...
int AnaEffHodo::process_event(PHCompositeNode* topNode)
{
	...
	int nElements[8] = {23, 23, 16, 16, 16, 16, 16, 16};
	/////////////////////////
	int det_ID = geom->getDetectorID(hodoNames[3]);
	...
	shared_ptr<SQHitVector> hv2(UtilSQHit::FindHits(hit_vec,hodoNames2[3]));  //hit vector of the opposite palnes  H2T <----> H2B 

	//if( hv2->size() !=0)continue;  //conditions opposite sides of the hodoscopes dont have any hit
	if( hv2->size() ==0 && (exp_element >=1 && exp_element <= nElements[3] ) ) {
		if(event->get_run_id() ==1764)h2xt_all[exp_element-1]->Fill(1);       //nom - 30 volts 
		//else if(event->get_run_id() ==1788)h2xt_all[exp_element-1]->Fill(2);  // nom 
		else if(event->get_run_id() ==1760)h2xt_all[exp_element-1]->Fill(2);  // nom 
		else if(event->get_run_id() ==1746)h2xt_all[exp_element-1]->Fill(3);  //nom + 30 volts
		else if(event->get_run_id() ==1763)h2xt_all[exp_element-1]->Fill(4);  //nom +60 volts
		else h2xt_all[exp_element-1]->Fill(5);  // current nominals

		shared_ptr<SQHitVector> hv(UtilSQHit::FindHits(hit_vec,hodoNames[3]));  //efficiency plane
		if( hv->size() >0){
			...
			if (minElement >=0 &&(abs(exp_element - hit_hx[minElementIndex])<=1)){
				if(event->get_run_id() ==1764)h2xt_acc[exp_element-1]->Fill(1);
				//else if(event->get_run_id() ==1788)h2xt_acc[exp_element-1]->Fill(2);
				else if(event->get_run_id() ==1760)h2xt_acc[exp_element-1]->Fill(2);
				else if(event->get_run_id() ==1746)h2xt_acc[exp_element-1]->Fill(3);
				else if(event->get_run_id() ==1763)h2xt_acc[exp_element-1]->Fill(4);
				else h2xt_acc[exp_element-1]->Fill(5);  // current nominals
			}
		}
	}	
	/////////////////////
...
int AnaEffHodo::End(PHCompositeNode* topNode)
{
	...
	//std::string Names[5] = {"old-nom-30", "old-nom", "old-nom +30", "old-nom +60", "curent-running"};

	for(int i = 0; i < 16; ++i)    {
		eff_h2xt[i] = new TEfficiency(*h2xt_acc[i], *h2xt_all[i]);
		oss.str("");
		oss2.str("");
		oss << HodopaddleNames[i]->GetName()<<".png";
		oss2 << HodopaddleNames[i]->GetName();
		eff_h2xt[i]->SetName(oss.str().c_str());
		eff_h2xt[i]->SetMarkerColor(kRed);
		eff_h2xt[i]->SetLineColor  (kRed);
		eff_h2xt[i]->SetStatisticOption(TEfficiency::kBBayesian);
		eff_h2xt[i]->SetConfidenceLevel(0.68);
		eff_h2xt[i]->SetMarkerStyle(20);
		eff_h2xt[i]->SetTitle(";Paddles;Paddle Efficiency");
		eff_h2xt[i]->SetTitle(oss2.str().c_str());
		eff_h2xt[i]->Draw("APE1");
		c1->Update();
		c1->Pad()->Update();
		TGraphAsymmErrors *gg = eff_h2xt[i]->GetPaintedGraph();
		TAxis *ay = gg->GetYaxis();
		TAxis *ax = gg->GetXaxis();
		ay->SetRangeUser(0,1.0); // overwrite auto
		//vector <string > names={"old-nom-30", "old-nom", "old-nom +30", "old-nom +60", "curent-running" };
      		//for (Int_t k=0;k<16;k++) ax->SetBinLabel(k+1,names[k].c_str());
		//oss.str("");
                //oss << HodopaddleNames[i]->GetName()<<".png";
